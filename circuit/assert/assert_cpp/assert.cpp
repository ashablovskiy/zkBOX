...
void assesDeposit_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void assesDeposit_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "assesDeposit";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 4;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
Fr_eq(&expaux[1],&lvar[0],&signalValues[mySignalStart + 1]); // line circom 19
Fr_geq(&expaux[2],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 0]); // line circom 19
...
assesDeposit_2_create(1,0,ctx,"main",0);
assesDeposit_2_run(0,ctx);
}
