...
void DualMux_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void DualMux_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
void HashLeftRight_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void HashLeftRight_4_run(uint ctx_index,Circom_CalcWit* ctx);
void Mint_5_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Mint_5_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 12434;}

uint get_number_of_components() {return 29;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 9259;}
...
FrElement lvar[0];
uint sub_component_aux;
Fr_sub(&expaux[3],&circuitConstants[3],&signalValues[mySignalStart + 4]); // line circom 27
Fr_mul(&expaux[1],&signalValues[mySignalStart + 4],&expaux[3]); // line circom 27
Fr_eq(&expaux[0],&expaux[1],&circuitConstants[1]); // line circom 27
...
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 2]); // line circom 28
Fr_mul(&expaux[1],&expaux[2],&signalValues[mySignalStart + 4]); // line circom 28
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 2]); // line circom 28
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 1];
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 3]); // line circom 29
Fr_mul(&expaux[1],&expaux[2],&signalValues[mySignalStart + 4]); // line circom 29
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 3]); // line circom 29
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
}
...
ctx->componentMemory[coffset].templateName = "Mint";
ctx->componentMemory[coffset].signalStart = soffset;
...
uint csoffset = mySignalStart+7105;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimcC";
mySubcomponents[aux_create+i] = aux_cmp_num;
...
mySubcomponents[aux_create+i] = aux_cmp_num;
HashLeftRight_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
}
}
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + 2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0]); // line circom 53
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[9]); // line circom 65
while(Fr_isTrue(&expaux[0])){
Fr_eq(&expaux[0],&lvar[1],&circuitConstants[1]); // line circom 67
if(Fr_isTrue(&expaux[0])){
{
...
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
DualMux_2_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
Fr_sub(&expaux[0],&lvar[1],&circuitConstants[3]); // line circom 67
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&expaux[0])) + 6)]].signalStart + 0]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
DualMux_2_run(mySubcomponents[cmp_index_ref],ctx);
...
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
HashLeftRight_4_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[3]); // line circom 65
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[9]); // line circom 65
}
Fr_eq(&expaux[0],&signalValues[mySignalStart + 1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[9]].signalStart + 0]); // line circom 76
