...
void HashLeftRight_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void HashLeftRight_3_run(uint ctx_index,Circom_CalcWit* ctx);
void MerkleTreeChecker_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MerkleTreeChecker_4_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[5] = { 
DualMux_0_run,
MiMCFeistel_1_run,
MiMCSponge_2_run,
HashLeftRight_3_run,
MerkleTreeChecker_4_run };
uint get_main_input_signal_start() {return 2;}

uint get_main_input_signal_no() {return 6;}

uint get_total_signal_no() {return 3564;}

uint get_number_of_components() {return 11;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 2651;}
...
Fr_sub(&expaux[3],&circuitConstants[0],&signalValues[mySignalStart + 4]); // line circom 28
Fr_mul(&expaux[1],&signalValues[mySignalStart + 4],&expaux[3]); // line circom 28
Fr_eq(&expaux[0],&expaux[1],&circuitConstants[1]); // line circom 28
...
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 2]); // line circom 29
Fr_mul(&expaux[1],&expaux[2],&signalValues[mySignalStart + 4]); // line circom 29
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 2]); // line circom 29
// end load src
...
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 3]); // line circom 30
Fr_mul(&expaux[1],&expaux[2],&signalValues[mySignalStart + 4]); // line circom 30
...
Fr_lt(&expaux[0],&lvar[221],&circuitConstants[2]); // line circom 275
...
uint csoffset = mySignalStart+3553;
uint aux_dimensions[1] = {2};
for (uint i = 0; i < 2; i++) {
...
mySubcomponents[aux_create+i] = aux_cmp_num;
HashLeftRight_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
}
}
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[5]); // line circom 51
while(Fr_isTrue(&expaux[0])){
Fr_eq(&expaux[0],&lvar[1],&circuitConstants[1]); // line circom 53
if(Fr_isTrue(&expaux[0])){
{
...
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
HashLeftRight_3_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[0]); // line circom 51
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[5]); // line circom 51
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + 0],&signalValues[mySignalStart + 2]); // line circom 69
...
MerkleTreeChecker_4_create(1,0,ctx,"main",0);
MerkleTreeChecker_4_run(0,ctx);
}
