...
void Redeem_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Redeem_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 5313;}

uint get_number_of_components() {return 9;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 3963;}
...
ctx->componentMemory[coffset].templateName = "Redeem";
ctx->componentMemory[coffset].signalStart = soffset;
...
uint csoffset = mySignalStart+2658;
for (uint i = 0; i < 1; i++) {
...
Redeem_2_create(1,0,ctx,"main",0);
Redeem_2_run(0,ctx);
}
