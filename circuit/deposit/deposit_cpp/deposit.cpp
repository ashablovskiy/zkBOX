...
void hashDeposit_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void hashDeposit_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 1774;}
...
ctx->componentMemory[coffset].templateName = "hashDeposit";
ctx->componentMemory[coffset].signalStart = soffset;
...
hashDeposit_2_create(1,0,ctx,"main",0);
hashDeposit_2_run(0,ctx);
}
